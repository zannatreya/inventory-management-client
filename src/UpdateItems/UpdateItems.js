...
const UpdateItems = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await axios.get(
        `https://inventory-management-server-xi.vercel.app/items/${id}`
      );
      setInventory(data);
    })();
  }, [inventory, id]);
  ...
  return (
    <div className="px-4 md:px-20">
      <section className="py-10 text-white grid grid-cols-1 md:grid-cols-3">
        {/* item details */}
        <article className="col-span-2">
          ...
          <div className="px-4 pb-4">
            <div className="flex flex-col md:flex-row justify-between items-center gap-5">
              ...
              {quantity === 0 ? (
                <button
                  ...
                  className={`bg-cyan-500 border border-cyan-500 py-2 px-8 rounded-md cursor-not-allowed opacity-20`}
                  disabled
                >
                  Delivered
                </button>
            ...
            <button
              ...
              className="hover:bg-cyan-500 bg-cyan-600 py-3 px-10 rounded-md duration-200 ease-in-out"
            >
              Manage Inventories
            </button>
          </div>
        ...
        <article className="p-5 ">
          <div className="bg-cyan-500 text-slate-800 rounded-xl p-5 sticky-top top-20 -z-0">
            ...
            <form onSubmit={handleAddItem} className="mt-10 mb-3">
              <input
                className="block w-full bg-transparent border-b border-slate-800 text-slate-800 outline-none placeholder:text-slate-800 "
                type="number"
                name="number"
              ...
              />
              <input
                className="hover:bg-white border hover:text-cyan-500 text-white py-2 px-6 mt-8 rounded-md font-semibold duration-200 ease-in-out"
                type="submit"
                value="Add Item"
              />
            </form>
